...
$uin = '18810102180511993457';

$result = uinIsValidate($uin);

var_dump($result);

function uinIsValidate($uin) {
	...
	$controlSumAndRound = getControlAndRound($arrNumbers, 1, 0);
	...
	if ($controlSumAndRound[1] == 1 && ($controlSumAndRound[0] % 11) !== 10) {
		...
		$controlSumAndRound = getControlAndRound($arrNumbers, 2, 2);
	...
	return finish($controlChar, $lastChar, $round);

}

function getControlAndRound($arrNumbers, $round, $shiftLeft) {
...
function finish($controlChar, $lastChar, $round) {
